...
class RelativeAxisItem(pg.AxisItem):
    ...
    def setOffset(self, offset):
        if self.offset != offset:
            self.offset = offset
            self.picture = None # Force regeneration of ticks/labels
            self.update()

    def tickStrings(self, values, scale, spacing):
        # Subtract the offset from the true coordinate values to get the label values
...
class ADCViewer(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # --- UI Setup ---
        self.setWindowTitle("Pico 2 W 2-Channel Oscilloscope PRO v3")
        ...
        self.plot_widget.setYRange(-1.0, 4.3)
        self.plot_widget.setMouseEnabled(x=False, y=False)
        self.plot_widget.hideButtons()
        
        # Global Reference Line (0V level indicator) - Optional
        self.plot_widget.addLine(y=0, pen=pg.mkPen((50,50,50), width=1, style=QtCore.Qt.PenStyle.DotLine))
        ...
        self.ch1_offset.valueChanged.connect(self.update_ui_state)
        ...
        self.ch2_offset.valueChanged.connect(self.update_ui_state)
    ...
    def set_active_channel(self, ch):
        ...
        self.y_axis.setPen(pg.mkPen(color, width=1))
        self.y_axis.setTextPen(pg.mkPen(color))
        
        # Update GroupBox highlight
    ...
    def update_ui_state(self):
        ...
        self.m1.setValue(self.ch1_offset.value())
        self.m2.setValue(self.ch2_offset.value())
    ...
    def update_data(self):
        ...
        try:
            ...
            if src != "None":
                ...
                if edge == "Rising":
                    indices = np.where((trig_v[:500-1] < level) & (trig_v[1:500] >= level))[0]
                else:
                    indices = np.where((trig_v[:500-1] > level) & (trig_v[1:500] <= level))[0]
            ...
            time_array = (np.arange(len(ch1_f)) - (len(ch1_f)/2 if src=="None" else 0)) * t_step
